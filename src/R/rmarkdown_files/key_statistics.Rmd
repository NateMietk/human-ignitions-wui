...
# Number/burned area of large wildfires size versus region
```{r eval=TRUE}
as_tibble(as.data.frame(fpa_wui_df)) %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other'))) %>%
  transform(class = factor(class, levels=c('Intermix WUI', 'Interface WUI', 'VLD', 'Wildlands'))) %>%
  mutate(fire_size = case_when(
    ...
    TRUE ~ NA_character_ )) %>%
  group_by(fire_size, region, class, ignition) %>%
  ...
  arrange(desc(region, class, fire_size)) %>%
...
t1 <- as_tibble(as.data.frame(fpa_bae_wui_df)) %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other', 'Wildlands'))) %>%
...
t2 <- as_tibble(as.data.frame(wui_209_df)) %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other', 'Wildlands'))) %>%
  filter(cause != 'Unk') %>%
  group_by(cause) %>%
  summarise(costs = sum(costs)) %>%
...
as_tibble(as.data.frame(fpa_wui_df)) %>%
  ...
  mutate(pct_frequency = fire_frequency/sum(fire_frequency)*100,
         pct_burned_area = wildfire_burned_area/sum(wildfire_burned_area)*100) %>%
  mutate_if(is.numeric, funs(round(., 2))) %>%
  left_join(., t1, by = c('cause')) %>%
  left_join(., t2, by = c('cause')) %>%
  left_join(., t3, by = c('cause')) %>%
  gather(var, value, -cause) %>% 
  spread(cause, value) %>%
  mutate(pct_human = round(Human/(Human+Lightning)*100,0)) %>%
  mutate_if(is.numeric, funs(round(., 2))) %>%
  mutate_if(is.numeric, funs(format(., scientific=F))) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = 'left', font_size = 10)
```
# What is the median discovery date
```{r eval=TRUE}
as_tibble(as.data.frame(fpa_wui_df)) %>%
  group_by(ignition) %>%
  summarise(median_doy = median(discovery_doy))
```
# What is the spring fire freq and burned area
```{r eval=TRUE}
as_tibble(as.data.frame(fpa_wui_df)) %>%
  filter(class_coarse == 'WUI') %>%
  group_by(seasons, class_coarse, ignition) %>%
  summarise(freq = n(),
            burned_area = sum(fire_size_km2),
            median_doy = median(discovery_doy))
```
# What is the median doy for fall fires
```{r eval=TRUE}
as_tibble(as.data.frame(fpa_wui_df)) %>%
  filter(seasons == 'Fall') %>%
  group_by(ignition) %>%
...
mtbs_fitted_0 <- as_tibble(as.data.frame(wui_209_df)) %>%
  ...
  group_by(start_year) %>%
  summarise(costs = sum(costs)) %>%
  mblm(costs ~ start_year, data = ., repeated = FALSE)
...
as_tibble(as.data.frame(wui_209_df)) %>%
  ...
  group_by(start_year) %>%
  summarise(costs = sum(costs)) %>%
  ggplot(aes(x = start_year, y = costs)) +
...
as_tibble(as.data.frame(fpa_wui_df)) %>%
  ...
  spread(cause, value) %>%
  mutate(pct_human = round(Human/(Human+Lightning)*100,0)) %>%
  mutate(Human = format(Human, scientific=F),
         Lightning = format(Lightning, scientific=F)) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = 'left', font_size = 10)
```
#### Table 3b
...
as_tibble(as.data.frame(fpa_wui_df)) %>%
  ...
  mutate(pct_frequency = fire_frequency/sum(fire_frequency)*100,
         pct_burned_area = wildfire_burned_area/sum(wildfire_burned_area)*100) %>%
  mutate_if(is.numeric, funs(round(., 2))) %>%
  left_join(., t1, by = c('cause', 'class')) %>%
  left_join(., t2, by = c('cause', 'class')) %>%
  left_join(., t3, by = c('cause', 'class')) %>%
  unite(class_cause, class, cause) %>%
  gather(var, value, -class_cause) %>% 
  spread(class_cause, value) %>%
  mutate(pct_intermix = round(`Intermix WUI_Human`/(`Intermix WUI_Human`+`Intermix WUI_Lightning`)*100,0),
         pct_interface = round(`Interface WUI_Human`/(`Interface WUI_Human`+`Interface WUI_Lightning`)*100,0),
         pct_vld = round(VLD_Human/(VLD_Human+VLD_Lightning)*100,0),
         pct_wildland = round(Wildlands_Human/(Wildlands_Human+Wildlands_Lightning)*100,0)) %>%
  dplyr::select(var, `Intermix WUI_Human`, `Intermix WUI_Lightning`, pct_intermix, `Interface WUI_Human`, `Interface WUI_Lightning`, pct_interface,
                VLD_Human, VLD_Lightning, pct_vld, Wildlands_Human, Wildlands_Lightning, pct_wildland) %>%
  mutate_if(is.numeric, funs(format(., scientific=F))) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F, position = 'left', font_size = 10)
