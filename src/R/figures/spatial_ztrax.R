...
fire_density <- bu_cleaned %>%
  group_by(fishid25k, ignition) %>%
  summarise(fcnt = n()) %>%
  ungroup() %>%
  filter(!is.na(ignition)) %>%
  spread(ignition, fcnt) %>%
  left_join(., fishnet_25k, by = "fishid25k") %>%
  as.data.frame() %>%
...
bu_df <- bu_cleaned %>%
  ...
  left_join(fs25_df, ., by = "fishid25k") %>%
  left_join(fire_density, ., by = "fishid25k") %>%
  na.omit()
...
p1 <- bu_df %>%
  ...
  scale_size_discrete(range = c(0.15, 2.5), name="# of homes \nthreatened") +
  theme_map() +
  ...
  theme(plot.title = element_text(hjust = 0, size = 12),
        ...
        legend.key = element_rect(fill = "white")) + facet_wrap(~ignition, ncol = 1)
