bu_complete_long <- as_tibble(as.data.frame(bu_complete_cleaned)) %>%
  dplyr::select(fpa_id, class, ignition, discovery_year, built_class, 
                build_up_count_0, build_up_count_250, build_up_count_500, build_up_count_1000) %>%
  gather(key = 'buffer_class', value = 'built_count', -fpa_id, -class, -ignition, -discovery_year, -built_class) %>%
  mutate(buffer_class = case_when(
    buffer_class == 'build_up_count_0' ~ 'Fire perimeter',
    buffer_class == 'build_up_count_250' ~ 'Buffer zone - 250m',
    buffer_class == 'build_up_count_500' ~ 'Buffer zone - 500m',
    buffer_class == 'build_up_count_1000' ~ 'Buffer zone - 1000m',
    TRUE ~ NA_character_))
...
mtbs_fitted_0 <- bu_complete_long %>%
  filter(buffer_class == 'Fire perimeter') %>%
  filter(built_class == 'Residential') %>%
  droplevels() %>%
  group_by(discovery_year) %>%
  summarise(sum_built_count = sum(built_count)/1000000) %>%
...
mtbs_fitted_250 <- bu_complete_long %>%
...
mtbs_fitted_500 <- bu_complete_long %>%
...
mtbs_fitted_1000 <- bu_complete_long %>%
...
class_fpa_bu_yearly <- bu_complete_long %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other'))) %>%
  filter(built_class == 'Residential') %>%
  droplevels() %>%
  group_by(class) %>%
...
lt_class_fpa_bu_yearly <- bu_complete_long %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other'))) %>%
  filter(built_class == 'Residential') %>%
...
class_fpa_bu_df <- bu_complete_long %>%
  filter(!(class %in% c("High Urban", "Med Urban", "Low Urban", 'Other'))) %>%
  filter(built_class == 'Residential') %>%
