...
ics_totals <- as.data.frame(wui_209) %>%
  ...
  group_by(fishid25k, class_coarse) %>%
...
wui_fish25k_sum <- ungroup(wui_fish25k_sum) %>%
  filter(year == 2010) %>%
...
fire_density <- as.data.frame(fpa_wui) %>%
  group_by(fishid25k, ignition, class_coarse) %>%
  summarise(n_fire = n()) %>%
  ungroup() %>%
  spread(ignition, n_fire) %>%
  left_join(., class_totals, by = c("class_coarse", "fishid25k")) %>%
...
bu_density <- as.data.frame(bu_complete_cleaned) %>%
  ...
  left_join(., bu_totals, by = c("class_coarse", "fishid25k")) %>%
  mutate(n_Human = ifelse(is.na(Human), 0, Human),
         n_Lightning = ifelse(is.na(Lightning), 0, Lightning),
...
ics_density <- as.data.frame(wui_209) %>%
  group_by(fishid25k, cause, class_coarse) %>%
  summarise(costs = sum(costs)) %>%
  ungroup() %>%
  spread(cause, costs) %>%
  left_join(., ics_totals, by = c("class_coarse", "fishid25k")) %>%
  mutate(n_Human = ifelse(is.na(Human), 0, Human),
         n_Lightning = ifelse(is.na(Lightning), 0, Lightning),
...
burn_area_density <- as.data.frame(fpa_bae_wui) %>%
  ...
  mutate(s_Human = ifelse(is.na(Human), 0, Human),
         ...
         pct_class_ba = tot_fire/total_fishid25k_area*100,
...
p3 <- conus_bu %>%
  ...
  scale_size_discrete(range = c(.2, 0.9), name="Homes threatened") +
  theme_nothing(legend = TRUE) +
