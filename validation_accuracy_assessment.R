...
if(!file.exists(file.path(geomac_dir, 'geomac_fpa.gpkg'))) {
  ...
  geomac_fpa_final <- geomac_fpa_filtered %>%
  ...
  st_write(geomac_fpa_final, file.path(geomac_dir, 'geomac_fpa.gpkg'))
} else {
  geomac_fpa_final <- st_read(file.path(geomac_dir, 'geomac_fpa.gpkg'))
}

if(!file.exists(file.path(geomac_dir, 'mtbs_fpa.gpkg'))) {
  ...
  st_write(mtbs_fpa_final, file.path(geomac_dir, 'mtbs_fpa.gpkg'))
  
  } else {
    mtbs_fpa_final <- st_read(file.path(geomac_dir, 'mtbs_fpa.gpkg'))
  }

fpa_mtbs_geomac <- geomac_fpa_final  %>%
  full_join(., as.data.frame(mtbs_fpa_final) %>% dplyr::select(-geom)) %>%
...
if (!file.exists(file.path(fire_poly, "fpa_mtbs_geomac_bae.gpkg"))) {
  # Create the distance variable to create the simple buffer
  fpa_geom <- fpa_fire %>%
    dplyr::select(FPA_ID, geom)
  
  fpa_mtbs_geomac_bae <- fpa_geom %>%
    left_join(as.data.frame(fpa_mtbs_geomac) %>%
                dplyr::select(-Shape), ., by = 'FPA_ID') %>%
    st_as_sf() %>%
    mutate(FIRE_SIZE_M2 = pmax(FPA_ACRES, MTBS_ACRES, GEOMAC_ACRES, na.rm = TRUE)*4046.86,
           RADIUS = sqrt(FIRE_SIZE_M2/pi)) %>%
    sf::st_transform(proj_ed) %>%
    st_buffer(., dist = .$RADIUS) %>%
    st_transform(proj_ea)
  
  st_write(fpa_mtbs_geomac_bae, file.path(fire_poly, "fpa_mtbs_geomac_bae.gpkg"),
           driver = "GPKG", delete_layer = TRUE)
  } else {
    fpa_mtbs_geomac_bae <- st_read(file.path(fire_poly, "fpa_mtbs_geomac_bae.gpkg")) 
  }
